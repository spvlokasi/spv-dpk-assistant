...
export const buildSmartFase1 = (branchId: string, timestamp: number, ctx: any): ActionPlanMilestone[] => {
  ...
  if (ctx.hasElectricityIssue) {
    phase1TasksWeek1.push({ id: `t-${timestamp}-101`, title: `⚡ Pengaturan suhu AC 24-25°C & matikan 1 unit saat sepi guna pangkas opex ke batas ${formatShortRupiah(ctx.targetOpex)}/bln`, assignedTo: 'KTB', frequency: 'harian', completed: false, verifiedBySpv: false });
  ...
  return [
    {
      ...
      targetMetric: `Pangkas Opex <= ${formatShortRupiah(ctx.targetOpex)}/bln & Nol Dead Stock`, status: 'in_progress', tasks: phase1TasksWeek1
    },
    {
