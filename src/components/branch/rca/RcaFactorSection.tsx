    ...
    <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4 shadow-lg">
      <div className="space-y-3">
        {factors.length === 0 ? (
          <div className="py-7 px-4 text-center rounded-xl bg-slate-850/40 border border-dashed border-slate-800 space-y-1.5">
            <p className="text-xs font-semibold text-slate-400">{isInternal ? 'Belum ada faktor diagnosa internal' : 'Belum ada faktor diagnosa eksternal'}</p>
            <p className="text-[11px] text-slate-500">{isInternal ? <>Klik <span className="text-emerald-400 font-semibold">"Muat Standar"</span> atau <span className="text-emerald-400 font-semibold">"+ Faktor"</span>.</> : <>Klik <span className="text-amber-400 font-semibold">"+ Faktor"</span>.</>}</p>
          </div>
        ) : (
          factors.map((factor) => (
            <RcaFactorItem key={factor.id} factor={factor} onUpdate={(field, val) => onUpdateFactor(factor.id, field, val)} onDelete={() => onDeleteFactor(factor.id)} />
          ))
        )}
      </div>
      ...
      <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-slate-300">{isInternal ? 'Diagnosa Faktor Internal' : 'Diagnosa Faktor Eksternal'}</span>
