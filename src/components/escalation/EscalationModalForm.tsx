...
export const EscalationModalForm: React.FC<EscalationModalFormProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const ticket: EscalationTicket = {
      ...
      category: (editingTicket?.category || 'sdm_rotasi') as any,
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-xl w-full p-5 sm:p-6 space-y-4 shadow-2xl relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            ...
            <textarea rows={2} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Jelaskan detail kendala dan dampak kerugian..." className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-emerald-500" />
          </div>

          <div>
            <label className="block text-slate-400 mb-1 font-semibold">Usulan / Rekomendasi Solusi SPV:</label>
            <textarea rows={2} value={formData.proposedSolution} onChange={(e) => setFormData({ ...formData, proposedSolution: e.target.value })} placeholder="Usulan tindakan yang diharapkan..." className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-slate-200 focus:outline-none focus:border-emerald-500" />
          </div>

          {editingTicket && (
            <div>
              <label className="block text-slate-400 mb-1 font-semibold">Tanggapan / Disposisi Manajer:</label>
              <textarea rows={2} value={formData.managerFeedback} onChange={(e) => setFormData({ ...formData, managerFeedback: e.target.value })} placeholder="Instruksi / persetujuan Manajer..." className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-emerald-400 focus:outline-none focus:border-emerald-500" />
            </div>
          )}
