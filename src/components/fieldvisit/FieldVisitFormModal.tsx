...
export const FieldVisitFormModal: React.FC<FieldVisitFormModalProps> = ({
  ...
  const [formData, setFormData] = useState<FieldVisit>({
    ...
    spvName: editingVisit?.spvName || 'Supervisor DPK',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.branchId || !formData.agenda) {
      showToast('Pilih cabang dan isi agenda kunjungan!', 'warning');
      return;
    }
    onSave(formData);
    showToast(editingVisit ? 'Log kunjungan berhasil diperbarui!' : 'Log kunjungan & coaching berhasil disimpan!', 'success');
    onClose();
  };
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-2xl w-full p-5 sm:p-6 space-y-4 shadow-2xl my-6 relative">
        ...
        <div>
          <h3 className="text-base font-bold text-white">
            {editingVisit ? 'Edit Log Kunjungan Toko' : 'Catat Kunjungan & Coaching Lapangan'}
          </h3>
          <p className="text-xs text-slate-400">Pencatatan agenda, materi pembinaan KTB, dan temuan deviasi toko</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <FieldVisitGeneralSection
            branches={branches}
            formData={formData}
            onFormChange={(u) => setFormData((p) => ({ ...p, ...u }))}
          />

          <FieldVisitIssueInputSection
            issues={formData.issues}
            onAddIssue={(issue) => setFormData((p) => ({ ...p, issues: [...p.issues, issue] }))}
            onRemoveIssue={(id) => setFormData((p) => ({ ...p, issues: p.issues.filter((i) => i.id !== id) }))}
          />
