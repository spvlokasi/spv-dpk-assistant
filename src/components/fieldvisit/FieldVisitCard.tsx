    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 space-y-4 shadow-xl hover:border-slate-700/80 transition-all">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded-md bg-slate-800 border border-slate-700 text-[11px] font-mono font-bold text-emerald-400">{branch?.code || 'CABANG'}</span>
            <h3 className="text-sm font-bold text-white tracking-tight">{branch?.name || 'Cabang Tidak Dikenal'}</h3>
          </div>
          <div className="flex items-center gap-3 text-xs text-slate-400">
            <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-slate-500" />{formatDateIndo(visit.date)}</span>
            <span>•</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
        <div className="bg-slate-850/60 p-3 rounded-xl border border-slate-800/80 space-y-1.5">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">🎯 Agenda & Coaching KTB:</span>
          <div className="font-semibold text-slate-200">{visit.agenda}</div>
          <p className="text-slate-400">{visit.katokCoachingTopic}</p>
          {visit.katokCommitment && (<div className="text-emerald-400 font-medium pt-1 border-t border-slate-800/80">🤝 Komitmen: "{visit.katokCommitment}"</div>)}
        </div>
