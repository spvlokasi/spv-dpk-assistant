...
export const FieldVisitLog: React.FC<FieldVisitLogProps> = ({
  ...
  const handleOpenAdd = () => { setEditingVisit(null); setShowModal(true); };
  const handleEdit = (visit: FieldVisit) => { setEditingVisit(visit); setShowModal(true); };
  ...
  return (
    <div className="space-y-6">
      <FieldVisitHeader branches={branches} filterBranchId={filterBranchId} onFilterChange={setFilterBranchId} searchQuery={searchQuery} onSearchChange={setSearchQuery} totalVisits={visits.length} openIssuesCount={openIssuesCount} onOpenAddModal={handleOpenAdd} />
      ...
      <div className="space-y-4">
        {filteredVisits.length === 0 ? (
          <div className="bg-slate-900 border border-dashed border-slate-800 rounded-2xl p-12 text-center space-y-3">
            ...
            <button type="button" onClick={handleOpenAdd} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-md active:scale-95">+ Catat Kunjungan</button>
          </div>
        ) : (
          filteredVisits.map((v) => (
            <FieldVisitCard key={v.id} visit={v} branch={branches.find((b) => b.id === v.branchId)} onEdit={handleEdit} onDelete={onDeleteVisit} onToggleIssueResolved={handleToggleIssueResolved} />
          ))
        )}
      </div>

      {showModal && (
        <FieldVisitFormModal branches={branches} editingVisit={editingVisit} initialBranchId={filterBranchId !== 'all' ? filterBranchId : undefined} onSave={onSaveVisit} onClose={handleCloseModal} />
      )}
    </div>
  );
};
