...
export const FieldVisitHeader: React.FC<FieldVisitHeaderProps> = ({
  ...
  totalVisits, openIssuesCount, onOpenAddModal, isKtb = false
}) => {
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl">
      <div className="flex items-center gap-2.5 flex-wrap">
        {!isKtb && branches.length > 1 ? (
          ...
          <span className="px-3 py-2 rounded-xl bg-slate-800 border border-emerald-700/60 text-emerald-400 font-bold text-xs shadow-sm flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5" />
            <span>[{currentBranch?.code}] {currentBranch?.name}</span>
          </span>
        )}

        <div className="relative">
          <Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" />
          <input type="text" value={searchQuery} onChange={(e) => onSearchChange(e.target.value)} placeholder="Cari agenda / catatan..." className="pl-9 pr-3 py-2 rounded-xl bg-slate-800 border border-slate-700 text-xs text-slate-200 focus:outline-none focus:border-emerald-500 w-44 sm:w-56" />
        </div>

        <div className="flex items-center gap-2 text-xs text-slate-400 pl-1 hidden md:flex">
          <span>Total Supervisi: <strong className="text-slate-200">{totalVisits}</strong></span>
          <span>•</span>
