...
export const PerformanceFilterBar: React.FC<PerformanceFilterBarProps> = ({
  branches, activeBranchId, onSelectBranch, selectedMonth, onSelectMonth,
  availableMonths, onOpenAddModal, onOpenImportModal
}) => {
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-900 border border-slate-800 p-3.5 sm:p-4 rounded-2xl shadow-xl">
      <div className="flex items-center gap-2.5 flex-wrap flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 bg-slate-800 border border-slate-700 rounded-xl px-2.5 py-1">
          ...
          <select value={selectedMonth} onChange={(e) => onSelectMonth(e.target.value)} className="bg-transparent text-slate-200 text-xs font-semibold focus:outline-none cursor-pointer py-1.5 pr-2">
            <option value="all" className="bg-slate-800">Semua Periode</option>
            {availableMonths.map((m) => (<option key={m.value} value={m.value} className="bg-slate-800">{m.label}</option>))}
          </select>
        </div>
      </div>

      <div className="flex items-center gap-2 flex-wrap flex-shrink-0">
        <button type="button" onClick={onOpenImportModal} className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-750 text-slate-200 text-xs font-semibold border border-slate-700 flex items-center gap-1.5 active:scale-95 shadow-sm">
          <Upload className="w-3.5 h-3.5 text-emerald-400" />
          <span>Excel / CSV</span>
        </button>
        <button type="button" onClick={onOpenAddModal} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center gap-1.5 shadow-md shadow-emerald-950 active:scale-95 flex-shrink-0">
