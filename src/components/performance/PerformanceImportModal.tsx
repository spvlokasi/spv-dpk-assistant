...
export const PerformanceImportModal: React.FC<PerformanceImportModalProps> = ({
  branches, activeBranch, onImport, onClose
}) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      if (entries.length === 0) {
        showToast('Format file tidak dikenali atau kosong!', 'error'); return;
      }
      setParsedData(entries);
      showToast(`Berhasil membaca ${entries.length} baris data Excel!`, 'success');
    };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-lg p-6 space-y-4 shadow-2xl animate-in fade-in zoom-in-95">
        ...
        <div className="bg-slate-850 p-3.5 rounded-2xl border border-slate-800 space-y-1.5 text-xs">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={() => { downloadPerformanceExcelTemplate(activeBranch); showToast('Template Excel diunduh!', 'success'); }} className="px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold flex items-center gap-1.5 shadow-md active:scale-95">
        ...
        <div className="bg-slate-850 p-3.5 rounded-2xl border border-slate-800 space-y-2 text-xs">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-700 hover:border-emerald-500 rounded-2xl p-5 text-center cursor-pointer bg-slate-900/50">
            ...
            <p className="text-xs text-slate-300 font-medium">{fileName || 'Klik untuk upload file Excel'}</p>
          </div>
          <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleFileUpload} className="hidden" />
        </div>

        {parsedData.length > 0 && (<div className="p-2.5 bg-emerald-950/40 border border-emerald-800/60 rounded-xl text-xs text-emerald-300 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" /> {parsedData.length} baris data rekap siap dimasukkan</div>)}
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-800">
          ...
          <button type="button" onClick={() => { onImport(parsedData); showToast(`Sukses mengimpor ${parsedData.length} data masal!`, 'success'); onClose(); }} disabled={parsedData.length === 0} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 text-white text-xs font-bold shadow-lg flex items-center gap-1.5"><Upload className="w-3.5 h-3.5" /> Simpan ({parsedData.length}) Data</button>
        </div>
      </div>
    </div>
  );
