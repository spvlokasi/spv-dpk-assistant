...
export const DashboardBranchCard: React.FC<DashboardBranchCardProps> = ({ branch, performance, milestones, onSelect }) => {
  ...
  return (
    <div onClick={onSelect} className="group relative overflow-hidden bg-slate-900 border border-slate-800 hover:border-emerald-500/60 p-5 rounded-2xl transition-all cursor-pointer hover:shadow-xl hover:shadow-emerald-950/20 space-y-3">
      {imgUrl && (
        <>
          <div className="absolute inset-0 bg-cover bg-center opacity-65 group-hover:opacity-85 transition-opacity duration-300 pointer-events-none rounded-2xl" style={{ backgroundImage: `url(${imgUrl})` }} />
      ...
      <div className="relative z-10 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-100 group-hover:text-emerald-400 transition-colors">{branch.name}</h4>
            <div className="text-[11px] text-slate-400">KTB: <span className="text-slate-300 font-medium">{branch.kepalaToko}</span> | SPV: {branch.spvArea}</div>
          </div>
        </div>
      ...
      <div className="min-h-[60px] flex flex-col justify-center">
        {hasDiagnosed ? (
          <div className="relative z-10 text-xs text-slate-300 bg-slate-850/85 p-2.5 sm:p-3 rounded-xl border border-slate-800/90 backdrop-blur-sm space-y-1">
            ...
            <div className="text-[11px] text-slate-300 line-clamp-1">{branch.diagnosisSummary || 'Faktor telah dinilai, siap susun strategi perbaikan.'}</div>
          </div>
        ) : (
      ...
      <div className="relative z-10 space-y-2.5 pt-2.5 border-t border-slate-800/80">
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className="font-bold text-slate-200 font-mono">
              ...
              <strong className={salesPct >= 100 ? 'text-emerald-400' : salesPct >= 80 ? 'text-amber-400' : 'text-rose-400'}>({salesPct}%)</strong>
            </span>
          </div>
          <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-500 ${salesPct >= 100 ? 'bg-emerald-500' : salesPct >= 80 ? 'bg-amber-500' : 'bg-rose-500'}`} style={{ width: `${Math.min(salesPct, 100)}%` }} />
          </div>
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className="font-bold text-slate-200 font-mono">{completedTasks}/{totalTasks} Tugas <strong className="text-blue-400">({taskPct}%)</strong></span>
          </div>
          <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden">
            <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${taskPct}%` }} />
          </div>
        </div>
      </div>
    </div>
