...
import { ToggleLeft, ToggleRight, Trash2, Calendar, Users, Building2, Tag } from 'lucide-react';
...
export const VoucherListItem: React.FC<VoucherListItemProps> = ({
  ...
  const hasSpecificProds = Boolean(v.applicableProductIds && v.applicableProductIds.length > 0);
  ...
  return (
    <div className={`bg-gradient-to-r from-amber-950/30 to-slate-900 border rounded-2xl p-4 flex flex-col justify-between gap-3 shadow-lg transition-all ${
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {hasSpecificProds && (
              <span className="px-1.5 py-0.5 rounded bg-emerald-950/80 border border-emerald-700/60 text-emerald-300 text-[10px] font-bold flex items-center gap-1">
                <Tag className="w-2.5 h-2.5" /><span>{v.applicableProductIds?.length} Produk Promo Pilihan</span>
              </span>
            )}
            {v.applicableCategory && v.applicableCategory !== 'all' && !hasSpecificProds && (
              <span className="px-1.5 py-0.5 rounded bg-amber-950/80 border border-amber-700/60 text-amber-300 text-[10px] font-bold">🏷️ {v.applicableCategory}</span>
            )}
            {isSponsor && (
              <span className="px-1.5 py-0.5 rounded bg-purple-950/80 border border-purple-700/60 text-purple-300 text-[10px] font-bold flex items-center gap-1">
                <Building2 className="w-2.5 h-2.5" /><span>{v.sponsorName || 'Sponsor'}</span>
              </span>
            )}
          ...
          {isSpv ? (
            <button type="button" onClick={() => onToggleStatus(v)} className="flex items-center gap-1 text-[11px] font-semibold text-slate-300 hover:text-white flex-shrink-0">
              {v.isActive ? <span className="text-emerald-400 flex items-center gap-1"><ToggleRight className="w-5 h-5 text-emerald-400" />Aktif</span> : <span className="text-slate-500 flex items-center gap-1"><ToggleLeft className="w-5 h-5" />Nonaktif</span>}
            </button>
          ) : (
            <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${v.isActive ? 'bg-emerald-950/80 border border-emerald-800/80 text-emerald-400' : 'bg-slate-800 text-slate-400'}`}>{v.isActive ? 'Aktif' : 'Nonaktif'}</span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 text-[11px]">
        <span className="text-[10px] text-slate-400">{isExpired ? '⚠️ Masa aktif berakhir' : remaining === 0 ? '❌ Kuota habis' : '✅ Siap diklaim konsumen'}</span>
        {isSpv && (
          <button type="button" onClick={() => onDelete(v)} className="p-1.5 rounded-lg bg-slate-800 hover:bg-rose-950/60 text-slate-400 hover:text-rose-400 transition-colors" title="Hapus Voucher"><Trash2 className="w-3.5 h-3.5" /></button>
        )}
      </div>
    </div>
  );
