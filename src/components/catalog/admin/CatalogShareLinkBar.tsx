...
export const CatalogShareLinkBar: React.FC<CatalogShareLinkBarProps> = ({ branch }) => {
  ...
  const shareUrl = `${window.location.origin}/?katalog=${branch?.code || 'M3017'}`;
  ...
  return (
    <div className="bg-gradient-to-r from-emerald-950/60 to-slate-900 border border-emerald-800/60 p-3.5 rounded-2xl flex flex-wrap items-center justify-between gap-2.5">
      <div className="min-w-0 flex-1"><span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider block">🔗 Link Publik Katalog Toko Ini:</span><div className="text-xs text-slate-200 font-mono font-semibold truncate select-all">{shareUrl}</div></div>
