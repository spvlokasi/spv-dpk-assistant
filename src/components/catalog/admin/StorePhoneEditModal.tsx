...
export const StorePhoneEditModal: React.FC<StorePhoneEditModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const updated: Branch = { ...branch, phone: phone.trim() };
      await StorageService.saveBranch(updated);
      showToast('Nomor WhatsApp Kasir Toko berhasil diperbarui!', 'success');
      onSaved();
      onClose();
    } catch {
      showToast('Gagal memperbarui nomor WhatsApp', 'error');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-sm w-full p-5 space-y-4 shadow-2xl animate-in fade-in zoom-in-95">
        <div className="flex items-center justify-between pb-2 border-b border-slate-800">
          <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm">
            <Phone className="w-4 h-4" /><span>Atur No. WhatsApp Kasir</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            <label className="block text-slate-400 mb-1 font-semibold">Nomor WhatsApp Penerima Order:</label>
            ...
            <p className="text-[11px] text-slate-400 mt-1">Pesanan pembeli di website belanja otomatis dikirim ke nomor WA ini.</p>
          </div>
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
            ...
            <button type="submit" disabled={saving} className="px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /><span>{saving ? 'Menyimpan...' : 'Simpan Nomor'}</span></button>
          </div>
        </form>
      </div>
    </div>
