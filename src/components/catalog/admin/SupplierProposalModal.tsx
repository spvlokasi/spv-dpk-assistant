...
import { FileText, Printer, Download, X, Copy, Check, Building2, Sparkles, Send, Award, Calendar, DollarSign, Target, CheckCircle2 } from 'lucide-react';
...
const PROPOSAL_PRESETS: ProposalPreset[] = [
  {
    ...
    summaryDescription: 'Peningkatan penetrasi konsumsi harian keluarga binaan TokoBasmalah melalui bundling sembako digital dan potongan harga khusus didanai Principal.'
  },
  {
    id: 'kanzler',
    ...
    targetOmzet: 9750000,
    fundingScheme: 'supplier',
    summaryDescription: 'Pemasaran intensif produk ready-to-eat & sosis viral Kanzler pada etalase katalog utama dengan voucher potongan harga instan via WhatsApp delivery.'
  },
  {
    id: 'unilever',
    ...
    targetOmzet: 8400000,
    fundingScheme: 'joint',
    summaryDescription: 'Program sharing margin 50:50 untuk mendongkrak basket size belanja ibu rumah tangga pada kategori fast moving home care.'
  }
];
...
export const SupplierProposalModal: React.FC<SupplierProposalModalProps> = ({
  ...
  const storeContribution = fundingScheme === 'store' ? totalSponsorBudget : fundingScheme === 'joint' ? totalSponsorBudget / 2 : 0;
  ...
  const handleCopyWA = () => {
    const text = `*PROPOSAL KERJASAMA TRADE PROMO TOKOBASMALAH*\n\n` +
      ...
      `*Supervisor DPK TokoBasmalah & Kepala Toko ${branch.name}*`;
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl rounded-3xl p-4 sm:p-6 space-y-5 shadow-2xl animate-in zoom-in-95 my-auto max-h-[95vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-800 no-print">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm sm:text-base font-extrabold text-white">Generator Proposal Kerjasama Supplier</h3>
              <p className="text-[11px] text-slate-400">Trade Marketing & Co-Marketing Partnership TokoBasmalah</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handlePrint}
              className="px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-blue-950/60"
            >
        ...
        {/* Panel Pemilihan Preset / Supplier (No Print) */}
        <div className="space-y-3 no-print bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-amber-400" />
              <span>Pilih Template Brand / Supplier Principal:</span>
            </label>
        ...
        {/* Parameter Customizer (Accordion Ringkas - No Print) */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs no-print bg-slate-950/40 p-3.5 rounded-2xl border border-slate-800/80">
          <div>
            <label className="block text-[10px] text-slate-400 font-semibold mb-1">Nama Perusahaan / Supplier</label>
            <input
              type="text"
              value={companyName}
              onChange={(e) => setCompanyName(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-2.5 py-1.5 text-slate-100 font-semibold focus:outline-none"
            />
          </div>
          <div>
            <label className="block text-[10px] text-slate-400 font-semibold mb-1">Brand / Merk Produk</label>
            <input
              type="text"
              value={brandName}
              onChange={(e) => setBrandName(e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-2.5 py-1.5 text-amber-300 font-semibold focus:outline-none"
            />
          </div>
          <div>
            <label className="block text-[10px] text-slate-400 font-semibold mb-1">Diskon / Voucher (Rp)</label>
            <input
              type="number"
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-2.5 py-1.5 text-slate-100 font-mono font-bold focus:outline-none"
            />
          </div>
        </div>

        {/* Action Bar Proposal Cepat (No Print) */}
        <div className="flex items-center justify-between gap-2 flex-wrap no-print pt-1">
          <div className="text-xs text-slate-300 font-semibold">
            Estimasi Anggaran Sponsor: <strong className="text-emerald-400 font-mono">{formatRupiah(supplierContribution)}</strong>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            >
              ...
              <span>{voucherInstalled ? '✓ Voucher Terpasang di Toko!' : '+ Pasang Voucher Ini ke Toko'}</span>
            </button>
          </div>
        </div>

        {/* DOKUMEN PROPOSAL RESMI (PRINTABLE LAYOUT) */}
          ...
          <div className="pt-6 border-t border-slate-300 font-sans">
            ...
            <div className="grid grid-cols-3 text-center gap-4 text-[11px]">
              <div>
                ...
                <strong className="block border-t border-slate-800 pt-1 text-slate-900">( M. Maskur )</strong>
              ...
              <div>
                ...
                <strong className="block border-t border-slate-800 pt-1 text-slate-900">( {branch.name} )</strong>
