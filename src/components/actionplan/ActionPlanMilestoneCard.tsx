    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg hover:border-slate-700">
      <div className="p-4 sm:p-5 flex items-center justify-between gap-3 border-b border-slate-850">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <button type="button" onClick={onDeleteMilestone} className="p-1.5 rounded-lg bg-slate-800/60 hover:bg-rose-950/60 text-slate-400 hover:text-rose-400 border border-slate-750" title="Hapus Milestone">
            <Trash2 className="w-3.5 h-3.5" />
          </button>
          <button type="button" onClick={onToggleExpand} className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300">
        ...
        <div className="p-4 sm:p-5 space-y-3 bg-slate-900/50">
          <div className="space-y-2">
            {milestone.tasks.map((task) => (
              <ActionPlanTaskItem key={task.id} task={task} onToggleComplete={() => handleUpdateTaskField(task.id, 'completed', !task.completed)} onToggleVerified={() => handleUpdateTaskField(task.id, 'verifiedBySpv', !task.verifiedBySpv)} onDelete={() => handleDeleteTask(task.id)} onUpdateTitle={(val) => handleUpdateTaskField(task.id, 'title', val)} onUpdateAssigned={(val) => handleUpdateTaskField(task.id, 'assignedTo', val)} onUpdateFrequency={(val) => handleUpdateTaskField(task.id, 'frequency', val)} />
            ))}
          </div>
          ...
          <form onSubmit={handleAddTask} className="flex items-center gap-2 pt-2">
            ...
            <button type="submit" disabled={!newTaskTitle.trim()} className="px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-bold active:scale-95 flex-shrink-0">
