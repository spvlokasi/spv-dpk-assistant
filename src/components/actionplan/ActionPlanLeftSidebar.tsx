    ...
    <div className="space-y-4 lg:sticky lg:top-4">
      <div className="bg-slate-900 border border-slate-800 p-4 sm:p-5 rounded-2xl shadow-xl space-y-4">
          ...
          {issueKeywords.length > 0 ? (
            <div className="flex items-center gap-1.5 flex-wrap">
              {issueKeywords.map((issue, idx) => (
                <span key={idx} className="px-2 py-0.5 rounded-md bg-amber-950/40 border border-amber-800/60 text-amber-300 text-[11px] font-semibold">⚠️ {issue}</span>
              ))}
            </div>
          ) : (
        ...
        <div className="space-y-2 text-xs pt-2 border-t border-slate-800">
          <div><span className="text-[11px] font-bold text-slate-400 block mb-1">Ringkasan Diagnosa:</span><p className="text-[11px] text-slate-300 leading-relaxed bg-slate-850/80 p-2.5 rounded-xl border border-slate-800/80">{branch.diagnosisSummary || 'Belum ada ringkasan diagnosa.'}</p></div>
          {branch.recommendedStrategy && (
            <div><span className="text-[11px] font-bold text-slate-400 block mb-1">Rekomendasi Strategi:</span><p className="text-[11px] text-slate-300 leading-relaxed bg-slate-850/80 p-2.5 rounded-xl border border-slate-800/80">{branch.recommendedStrategy}</p></div>
          )}
        </div>
