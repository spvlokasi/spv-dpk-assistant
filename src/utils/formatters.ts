...
export function parseAnyDateToIso(dateStr: string | number | null | undefined): string {
  ...
  if (typeof dateStr === 'number' || (/^\d{4,6}$/.test(String(dateStr).trim()) && Number(dateStr) > 30000 && Number(dateStr) < 70000)) {
    return new Date(Math.round((Number(dateStr) - 25569) * 86400 * 1000)).toISOString().slice(0, 10);
  }
...
export function formatDateIndo(dateStr: string | number | null | undefined): string {
  if (!dateStr) return '-';
  try {
    const d = new Date(parseAnyDateToIso(dateStr));
    return isNaN(d.getTime()) ? String(dateStr) : new Intl.DateTimeFormat('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }).format(d);
...
export function formatMonthYearIndo(ymStr: string): string {
  ...
  try {
    const [year, month] = parseAnyDateToIso(ymStr).slice(0, 7).split('-');
...
export const DPK_STATUS_PRIORITY: Record<string, number> = {
  akut: 1, kritis: 2, dalam_progres: 3, existing: 4, cabang_baru: 5, siap_lulus: 6, lulus_dpk: 7
};
